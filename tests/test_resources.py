...
# `per_sim_ram_gb` is passed EXPLICITLY throughout. Without it these inherit the learned calibration
# (cellarium.calibration), which is measured from whatever campaigns this host has run — so an assertion on
# `estimated_ram_gb` would pass or fail depending on machine history rather than on the arithmetic under test.
# The calibration path has its own coverage in tests/test_calibration.py.
_CONST_RAM = 2.0
...
def test_estimate_ok_when_it_fits():
    est = R.estimate_sim_resources(n_runs=4, parallel=2, generations=4, res=dict(_RES), per_sim_ram_gb=_CONST_RAM)
...
def test_estimate_warns_and_recommends_lower_parallel_when_ram_tight():
    est = R.estimate_sim_resources(n_runs=24, parallel=6, generations=4, res=dict(_RES), per_sim_ram_gb=_CONST_RAM)
...
def test_estimate_warns_and_chunks_when_disk_tight():
    ...
    est = R.estimate_sim_resources(n_runs=40, parallel=2, generations=4, res=tight, per_sim_ram_gb=_CONST_RAM)
...
def test_estimate_blocks_when_docker_down():
    ...
    est = R.estimate_sim_resources(n_runs=4, parallel=2, res=down, per_sim_ram_gb=_CONST_RAM)
...
def test_estimate_does_not_block_on_unknown_ram():
    ...
    est = R.estimate_sim_resources(n_runs=4, parallel=8, res=unknown, per_sim_ram_gb=_CONST_RAM)
    assert est["verdict"] == "ok" and not any("RAM" in w for w in est["warnings"])


def test_generations_scale_the_disk_estimate():
    a = R.estimate_sim_resources(n_runs=10, parallel=1, generations=4, res=dict(_RES), per_sim_ram_gb=_CONST_RAM)["estimated_disk_gb"]
    b = R.estimate_sim_resources(n_runs=10, parallel=1, generations=8, res=dict(_RES), per_sim_ram_gb=_CONST_RAM)["estimated_disk_gb"]
...
def test_effective_ram_is_capped_by_the_docker_vm(monkeypatch):
    ...
    est = R.estimate_sim_resources(n_runs=6, parallel=6, res=sr, per_sim_ram_gb=_CONST_RAM)       # would have looked fine vs host 20 GB
...
def test_wall_clock_warns_on_a_multi_day_sweep():
    ...
    est = R.estimate_sim_resources(n_runs=200, parallel=4, generations=8, res=res, per_sim_ram_gb=_CONST_RAM)
