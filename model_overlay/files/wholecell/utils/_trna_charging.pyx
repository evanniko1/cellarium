...
# EXT-PORT-12: libc.time is no longer cimported. The only use was `srand(time(NULL))` in the two
# reconcile kernels, which is exactly the defect that made the kinetic path non-reproducible; both
# now take an explicit `seed` argument. Removing the cimport is what stops it being reintroduced by
# a future verbatim merge from v3.0.1 without a compile error to notice it.
...
cpdef reconcile_via_ribosome_positions(
	...
	unsigned int seed, # 32-bit unsigned integer
	):

	# Set random seed
	# EXT-PORT-12 (UNIFY-2 gate): v3.0.1 seeds the C stdlib RNG from the WALL CLOCK here
	# (`cdef time_t t = time(NULL); srand(t)`), and rand() is then consumed below at four sites.
	# That makes every kinetic simulation non-reproducible: two runs with identical --seed diverge
	# at the first step in which the sequence limit and the kinetics limit disagree, which is
	# essentially every step. Measured, one full generation, same --seed 0, same KB, same image:
	# only 82 of 254 comparable listener columns matched, and Mass/cellMass was 0.51% apart at
	# division. The steady-state control matched in 250 of 274 columns (the 17 that differed were
	# EvaluationTime wall-clock timers). srand is re-called EVERY step, so consecutive steps landing
	# in the same wall-clock second also restart the SAME rand() stream -- the failure is not even
	# uniformly random.
	# The seed is now supplied by the caller, which draws it from the PROCESS RandomState (seeded
	# from the simulation seed at wholecell/sim/simulation.py:221-222). No behaviour changes except
	# that the sequence of rand() values becomes a deterministic function of --seed.
	srand(seed)
...
cpdef reconcile_via_trna_pools(
	...
	# Set random seed
	# EXT-PORT-12 (UNIFY-2 gate): see reconcile_via_ribosome_positions above. Same wall-clock
	# seeding in v3.0.1, same consequence, same fix. The caller draws a SECOND, independent value
	# from the process RandomState for this call so that reseeding here does not replay the stream
	# the first kernel just used within the same time step.
	srand(seed)
