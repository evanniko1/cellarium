...
class Metabolism(wholecell.processes.process.Process):
	...
	def initialize(self, sim, sim_data):
		...
		# Sim options
		# EXT-PORT-12 (UNIFY-2 gate): these two lines used to read ONE flag for TWO questions. See the
		# EXT-PORT-12 block in wholecell/sim/simulation.py for the split.
		#
		# use_trna_charging gates the amino-acid concentration TARGETS (lines 99-101 and 161-162
		# below). The question there is "does an elongation model consume amino acids into charged
		# tRNA?", which is true for the steady-state model and for KineticTrnaChargingModel alike --
		# so it takes _explicit_trna_charging. MEASURED under the old gate: conc_update_molecules was
		# 52 wide on the steady-state path and 32 on the kinetic path (20 amino-acid targets plus one
		# linked metabolite dropped, ppGpp added), i.e. the kinetic cell drained amino-acid pools all
		# generation against homeostatic targets that never moved.
		#
		# include_ppgpp asks a DIFFERENT question -- "is anything actually producing and degrading
		# ppGpp, so that FBA must not pin it?" -- and its value is UNCHANGED by this edit.
		# `not _ppgpp_driven_by_elongation` is exactly the old `not _ppgpp_regulation or not
		# _trna_charging`, because _ppgpp_driven_by_elongation is defined as their conjunction. It is
		# spelled separately so the parity model flips ppGpp on in ONE named place instead of by a
		# side effect of the charging flag.
		self.use_trna_charging = sim._explicit_trna_charging
		self.include_ppgpp = not sim._ppgpp_driven_by_elongation or getattr(metabolism, 'force_constant_ppgpp', False)
	...
	def evolveState(self):
		...
		if self.mechanistic_aa_transport:
			...
			polypeptide_elongation = self._sim.processes['PolypeptideElongation']
			# EXT-PORT-12 (UNIFY-2 gate): refuse to silently report zero uptake.
			#
			# aa_exchange_rates is written by SteadyStateElongationModel and by nothing else. Under a
			# kinetic elongation model it is still the zeros vector PolypeptideElongation.initialize
			# created, so without this check the FBA would be handed "uptake of every amino acid
			# present in the medium is exactly 0.0" -- a well-formed, plausible, wrong number, with no
			# exception and no warning. Measured harmless so far only because every run to date was
			# in '0 minimal', where aa_in_media is all False and the package is empty; the check is
			# written to fire exactly when that stops being true.
			#
			# Deliberately not a fallback to some estimated rate: there is no defensible estimate to
			# fall back TO, and inventing one is how a model runs and is quietly wrong.
			if aa_in_media.any() and not getattr(
					polypeptide_elongation, 'aa_exchange_rates_modelled', True):
				raise RuntimeError(
					'mechanistic_aa_transport is on and the medium contains amino acids ({}), but the'
					' selected elongation model never writes PolypeptideElongation.aa_exchange_rates,'
					' so metabolism would be told amino-acid uptake is exactly zero. Re-run with'
					' --no-mechanistic-aa-transport, or use an elongation model that computes'
					' exchange rates. See EXT-PORT-12 / BACKLOG UNIFY-2.'
					.format(list(self.aa_exchange_names[aa_in_media])))
			exchange_rates = (polypeptide_elongation.aa_exchange_rates
