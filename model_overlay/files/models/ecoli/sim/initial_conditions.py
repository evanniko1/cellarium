...
def calcInitialConditions(sim, sim_data):
	...
	# EXT-PORT-12 (UNIFY-2 gate): gated on _explicit_trna_charging, not _trna_charging. The question
	# this line asks is "does this simulation model charged tRNA as a distinct species?", which is
	# true for the steady-state model AND for KineticTrnaChargingModel -- not "which elongation
	# model was selected?", which is what _trna_charging answers (see wholecell/sim/simulation.py,
	# the EXT-PORT-12 block).
	#
	# MEASURED consequence of the old gate, one generation, seed 0, same KB: at t=0 the steady-state
	# cell had 140,590 charged tRNA (charged fraction 0.9795 of the total tRNA pool) and the kinetic
	# cell had ZERO (0.0000); 332 of 16,406 bulk species differed, 4,239,209 counts in total. The
	# kinetic pool relaxed to 0.780 charged after one step and ~0.83 by t=30, so it is a transient --
	# but it is a transient that starts the kinetic arm in maximal stringent-response territory at
	# exactly the moment a ppGpp comparison is most sensitive, and the steady-state arm never runs it.
	# Knock-on, same measurement: set_small_molecule_counts below then ran against a different cell
	# mass, giving WATER[c] +3,884,233, K+[c] +8,794, glycogen-monomer[c] +6,000, CPD-12819[c] +3,812.
	#
	# initialize_trna_charging partitions the EXISTING tRNA pool between charged and uncharged
	# species and debits the amino acids; it does not depend on which elongation model will run it,
	# and both models read the same charged-/free-tRNA bulk species.
	if sim._explicit_trna_charging:
