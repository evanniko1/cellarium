...
LIST_OF_DICT_FILENAMES = [
	...
	# EXT-PORT: kinetic tRNA charging inputs, from WholeCellEcoliRelease v3.0.1 (Choi & Covert 2023).
	# `raw_data` does NOT auto-scan flat/ — a file that is copied but not registered here is silently
	# invisible, which is how the first pass of this port "succeeded" while raw_data.optimization did
	# not exist at all.
