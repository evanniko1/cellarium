...
class SimulationDaughterTask(FiretaskBase):
	...
	optional_params = [
		...
		"translation_supply",
		"trna_charging",
		# EXT-PORT-3: the firetask keeps its OWN allow-list, separate from scriptBase.SIM_KEYS.
		# Keeping them in step is not optional: Fireworks raises on an unknown kwarg, so adding a
		# flag to scriptBase alone breaks EVERY run, including the default one.
