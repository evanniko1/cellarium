...
| **WELL-6s** | ⏳ **Index the 6 new runs.** They live under `runs/cellarium/gene_knockout_*/000000` but are **not in the manifest**, so `_design_run_roots` (manifest-driven) still cannot see them — the same orphan problem as WELL-6p, now self-inflicted. `ko_measured.json` carries the results durably, but `ko_verify.verify_corpus()` will keep reporting them unverified until they are indexed (`manifest.record_existing`). Fold into the WELL-6p reconciliation pass. | next | Our own runs are now orphans too |
