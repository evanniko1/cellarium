...
| **H-9** | P2 | Remaining leak-surface follow-ups from `wf_893d1bba`, none currently exploitable (the key is not in the agent's context, so it cannot be exfiltrated — these are defence for if that ever changes): (a) `skills.web_get(url, headers)` takes **model-supplied headers AND URL** — an exfil/SSRF primitive worth an allow-list on header names + an egress host allow-list; (b) `child_env()` so `subprocess.run` in `runner`/`reader`/`resources`/`provenance` passes an explicit env without `ANTHROPIC_API_KEY`; (c) a scrub in `SessionStore.put` and `launch.py`'s `str(exc)[:200]` persistence; (d) `record_demo_sessions.py --seed` should refuse to write the seed DB if a transcript carries a key-shaped string. | R |
