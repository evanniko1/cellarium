...
| **WELL-6i** | ⚠️ **CORRECTION to my own earlier claim — `pheS` status is UNKNOWN, not "not knocked out".** I stated that the aaRS ~gen-3 crash story "must drop pheS". That rested on `n_tu = 2` → predicted survival — which is **precisely the half of the rule that bamC refuted**. `pheS` has no local raw, so it was never measured. Its status is genuinely open. The aaRS story's *other* legs (argS, alaS, gltX, lysS) are also unmeasured, but they rest on `n_tu == 1` → silenced, the half with **zero counterexamples in 41** — so they are on materially firmer ground. **Owed:** measure pheS (a run exists in the manifest without local raw — either fetch it or re-run), before any manuscript sentence leans either way. | before the write-up | A prediction I let sound like a finding |
