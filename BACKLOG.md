...
| **WELL-6** | **Similarity retrieval — "runs whose response profile resembles this one".** The one scale query that is genuinely *not* SQL and *not* a graph: it is **ANN / vector** over the channel profile. This is the real "find me the relevant 200 of 10⁶ runs" primitive. | ≥ ~10⁴ runs, or when `survey_corpus` stops fitting in context | Needs an embedding choice (raw channel vector? normalised response? learned?) — a research decision, not plumbing |
...
| **WELL-11** | **MCP surface for third-party agents — OPEN, not decided.** Three tiers designed (`ask_cellwright` as one thick tool / a small read-only subset + Resources + Prompts / never-over-MCP), a sequence recommended, **nothing chosen**. Full write-up + lock-in analysis: [`docs/DECISIONS.md` **D6**](docs/DECISIONS.md). Key point: the irreversible commitment is the **schema** (WELL-1, WELL-9), not the protocol — settle design identity and factor columns before publishing any agent-facing surface. | the HF dataset ships, or someone asks to point an agent at the corpus | Decision owed |
