...
| **WELL-6f** | **⚠️ CORRECTNESS — the real find, and it is a JOIN, not an embedding. Partly fixed 2026-07-26.** `gene_knockout.py` calls `sim_data.adjust_final_expression([geneIndex], [0])`, and that index addresses a row of **`rna_data`** — a *transcription unit*, not a gene. On a polycistronic TU the whole operon is zeroed. Verified against the model's own `transcription_units.tsv`: **2,436 of 4,724 genes (52 %) sit on a multi-gene TU**, and **11 of the corpus's 21 single-KO designs** are affected — `flgB`→9 (flgBCDEFGHIJ), `rpmJ`→12 (incl. `secY`), `rplB`→11, `pheS`→8, `rpoB`→6, plus dnaN, glmS, dapA, lysS, selA, ymgD. **Three load-bearing project claims are confounded by this:** (a) `scope.py` uses `KO:flgB` as the canonical *"no phenotype BY CONSTRUCTION"* inert control — it is a nine-gene flagellar operon deletion; (b) the aaRS ~gen-3 crash generalisation rests partly on `pheS`, which simultaneously removes **infC (IF3), thrS (a second aaRS), pheT and ihfA**; (c) `rpoB`/`dnaN` survival is attributed to "inherited reserve" while both also lose operon partners (rpoB takes out `rpoC` + four ribosomal proteins) — a simpler competing explanation the codebase never considered. ✅ **Shipped:** `scope.ko_footprint()` + `data/cache/ko_footprint.json` (built by `scripts/build_ko_footprint.py`, committed so no checkout is needed at query time), surfaced through `classify_gene` → `mechanistic_scope` as an unmissable warning naming every co-silenced gene; `tests/test_ko_footprint.py`. ⏳ **Still owed:** re-read the three affected claims in `scope.py`/`generate.py` prose and re-word them; decide whether the affected designs need re-running as true single-gene KOs (they cannot be — the model has no monocistronic handle), or whether the corpus should simply relabel them as operon knockouts. | now | **This is what the graph question was actually worth** |
