...
| **DD-MTH-3** | MTH · DS · AGT | **Temperature-sweep eval** *(Tier 1)* | The Council's warm default (1.0) is a placeholder, not a measured optimum. Owe an eval that runs the **same canonical questions across a temperature grid** (non-reasoning model — the only regime where temperature is a lever, per the DD-MTH-2 note), with **replicates per (question, temperature)** so variance is measurable, scoring **hypothesis quality** (reuse the A/B min-bar / convergence / falsifier-quality) AND **cross-replicate diversity** — so the pinned Council temperature is *chosen empirically*, not asserted, and the exploration↔reproducibility trade-off is a reported curve. Establishes the right value for `COUNCIL_TEMPERATURE`. |
