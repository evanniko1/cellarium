...
|----|----------------------|--------------------|
| **SCI-TRNA-1** | **Per-family charged-tRNA fraction.** Today `fraction_trna_charged` is one aggregate number; the argS story depends on *which* isoacceptor family starves. | **Standard practice we lack — build it.** Elf, Nilsson, Tenson & Ehrenberg 2003, *Science* 300:1718, PMID 12805541 (selective charging: under limitation, charged levels approach zero for *some* isoacceptors while others stay charged); Dittmar 2005 measured the arginine-starvation case directly. The aggregate hides the mechanism. |
| **SCI-DYN-1** | **Treat shifts as transients, not per-segment means.** The `timeline` designs are nutrient shifts; collapsing each to segment means discards the adaptation dynamics. *(Directly adjacent to the design-identity bug fixed in `a1a0388` — the same runs were also being pooled.)* | **Standard practice we lack — build it.** Zhu & Dai 2023, *Nat Commun* 14:467, PMID 36709335. The strongest argument is internal: **Ahn-Horst 2022 is the wcEcoli extension whose entire purpose was dynamic shift response**, and whose headline result *is* a transient. Note `scan_series` (SP-2) already does transient detection on RAW series — this is the design-level analysis layer above it. |
...
| **SCI-GXE-1** | **Conditional essentiality** (gene × environment): a KO viable on rich and lethal on minimal. | **Build it — but the justification as written is factually wrong** and must be rewritten before publication (the claim about what Keio/Nichols cannot do does not hold). Baba 2006 PMID 16738554; Nichols 2011. |
| **SCI-DIL-1** | **Inherited-pool dilution** as the generation-dependent crash mechanism (the rpoB/argS `n(g)=n₀·2⁻ᵍ` story). | **Already published and named — calibrate, never claim.** "Phenotypic delay via dilution of sensitive molecules": Carballo-Pacheco et al. 2020, *PLoS Comput Biol* 16(5):e1007930, PMID 32469859. Presenting it as our discovery would be refuted. |
| **SCI-QC-1** | **Declared-vs-executed experiment check** — a design whose timeline declares two media events but whose run executed one must not pass QC silently. | **Standard practice we lack — build it, blocks publication.** **MIASE**: Waltemath et al. 2011, *PLoS Comput Biol* 7(4):e1001122, PMID 21552546 — the declared experiment description and the executed run must correspond. The design-identity bug was a MIASE violation; this is the general guard. |
| **SCI-FLUX-1** | **13C-MFA flux validation** of the sim's central-carbon fluxes. | **Build, but reframe.** Validating against ¹³C fluxes is ~20-year-old standard practice (Schuetz 2007) — the novelty is not the method, it is that it appears undone *for a whole-cell model*. Data: Gerosa et al. 2015, *Cell Syst* 1:270, PMID 27136056. |
...
| **WELL-6q** | **⚠️ The HF dataset repo is not reachable.** `huggingface_hub` (1.23.0, installed) returns **401 / RepositoryNotFound** for `evanniko1/cellarium-corpus`, so `hf.data_availability`'s "download from HF" alternative currently resolves to nothing — it correctly reports `available: false, verified: false` and steers to regeneration, which is the honest behaviour (DD-ENG-5 working as designed). But it means **raw pruned from disk is unrecoverable**, and it bears directly on **PUB-A5**: the dataset claim cannot be checked by a reader today. Determine whether the repo is private, unpushed, or renamed, and either publish or stop referencing it as an availability route. | before any dataset claim | The fallback we assumed exists, does not |
