...
| **WELL-6g** | **⚠️ CORRECTNESS — validated against REAL SIMULATION OUTPUT (no new sims needed), and it is worse than "operon knockouts".** **Mechanism (code-traced):** `gene_knockout.py` calls `sim_data.adjust_final_expression([i],[0])`; `adjust_final_expression` (`reconstruction/ecoli/simulation_data.py:314`) zeroes `rna_synth_prob[i]` and `rna_expression[i]`, vectors indexed over `rna_data` — **transcription units**, not genes. So one TU is zeroed. **Rule (exact, validated 27/27** against measured mRNA from existing local simOut, `KO:flgB`/`KO:rpmJ`/`KO:rpoB` vs `wildtype/basal`**): a gene is fully silenced IFF it has exactly one TU (`n_tu == 1`).** Three failure modes, all live: ① **real KO + collateral operon deletion** — `KO:flgB` measured **0.0 vs WT 5.8 for all nine** of flgBCDEFGHIJ, while flgA/flgK/flgM/fliC on other TUs are untouched (so it is the TU, not the regulon); also `KO:rplB` (+10 ribosomal proteins), `glmS`(+glmU), `selA`(+selB), `ymgD`(+ymgG). ② **the named gene is NOT knocked out** — `KO:rpoB` measured **rpoB mRNA 10.4 vs WT 8.4, i.e. no reduction at all**, because rpoB has 3 TUs and only one was zeroed. Same for `dnaN`(2), `murA`(2), `pheS`(2), `rpmJ`(2). **694 genes genome-wide are in this class.** ③ **the design silences a gene it is not named after** — `KO:rpmJ` measured rpmJ **still expressed** (50.1 vs 69.5) while **secY went to 0.0** (WT 15.8). **Two project explanations are refuted, not merely confounded:** (a) "rpoB/dnaN survive ≥4 gens on a large inherited RNAP/clamp reserve" — the simpler and measured explanation is that **neither gene was ever knocked out**; (b) the aaRS ~gen-3 crash generalisation must **drop `pheS`** (not knocked out) — it still stands on argS/alaS/gltX/lysS, which are all clean `n_tu==1` KOs. And `KO:flgB` as the "no phenotype BY CONSTRUCTION" inert control is a nine-gene flagellar operon deletion. ✅ **Shipped:** `scope.ko_footprint()` + committed `data/cache/ko_footprint.json` (2,607 non-clean of 4,724; built by `scripts/build_ko_footprint.py`), a per-failure-mode warning through `classify_gene`→`mechanistic_scope`, and `tests/test_ko_footprint.py` (9 tests) which **pins the measured counts as ground truth** so the rule cannot silently drift. Clean KOs (pfkA/argS/gltX/tpiA) correctly report nothing. ⏳ **Owed:** re-word the affected prose in `scope.py`/`generate.py`; decide whether the five non-KO designs get **relabelled** (they cannot be re-run as true single-gene KOs — the model exposes no monocistronic handle); and re-check any published/drafted claim that leans on rpoB, dnaN, pheS, rpmJ or murA. | **now — publication-blocking** | The graph audit's real yield |
