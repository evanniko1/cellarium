...
|----|-------------|--------------------|-------|
| **WELL-1** | **Declared factor schema — the prerequisite for everything below, and the one item that is cheap NOW and expensive later.** 150 of 155 `design.json` files carry `params == {"variant_index": N}` and nothing else. `variant_index: 2` means "0.6× ppGpp" under one perturbation and a transcription unit under another, resolvable only through hand-authored dicts in `generate.py:44`. Until factor NAME and LEVEL are first-class, no factorial query is expressible. ⚠️ Do **not** "just persist params and re-index": `record_existing()` spawns a Docker container per run and **117 of 265 manifest rows have no run directory on disk**, so a naive re-index ships a ~44 %-NULL factor column. Stamp factors going forward + backfill from `generate.py`'s dicts for the runs that still exist. | **Now** — every day of corpus growth makes the backfill worse | P1 if the HF dataset is the contribution |
...
| **WELL-6b** | **The embedding to test next: per-gene log2FC, not summary channels.** `differential.all_gene_lfc` already produces the full per-gene response vector (no significance filter, so no range restriction — shipped under SCI-2c). That is a ~4,000-dim mechanism fingerprint rather than a 10-dim severity score, and it is the *same representation* the connectivity-map literature uses for exactly this "which perturbation resembles which" query (CMap / L1000 connectivity score) — so there is established prior art for the metric, not just the plumbing. Also test: normalising each design's Δ-from-WT to unit length (removes the severity axis), and down-weighting near-null designs. **Validate against known biology as the acceptance test** — glycolytic KOs should cluster, and `wildtype` should NOT be a knockout's nearest neighbour. | now — needs no new dependency | The scientific result ("perturbations cluster by pathway") is worth as much as the retrieval capability |
