...
| **WELL-6i** | **⚠️ CORRECTION + measurement attempt (2026-07-26).** I stated the aaRS ~gen-3 story "must drop pheS". That rested on `n_tu = 2` → predicted survival — **precisely the half of the rule bamC refuted** — and pheS was never measured. Its status is UNKNOWN, not "not knocked out". **Measurement attempted for all 7 unverified designs; 1 succeeded.** ✅ **`gltX` MEASURED: SILENCED (0.0 vs WT 2.2**, null median 0.667 over 3,011 untargeted genes) — a genuine clean knockout, recovered from an **orphan run directory** (see WELL-6p). ❌ **alaS, argS, lysS, pfkA, pheS, rplB: raw is GONE.** Their run dirs survive but hold only `kb/` and `metadata/` — no seed output. HF cannot supply it either: `evanniko1/cellarium-corpus` returns **401 / RepositoryNotFound** (see WELL-6q). **The only route is a re-run, and it is cheap:** answering "is gene X silenced" needs the gen-0 mRNA readout, so **1 seed × 1 generation × 6 designs** ⇒ `estimate_sim_resources` says **4.0 GB RAM, 1.7 GB disk, ~0.4 h at parallel=2** — trivial against 14.7 GB free / 124 GB free. **Blocked only on Docker Desktop being started.** Until then: the aaRS story stands on `argS`/`alaS`/`lysS` (predicted clean, `n_tu==1`, the half of the prior with **zero counterexamples in 41**) plus **`gltX`, now measured**; `pheS` must simply not be cited either way. | one 24-min run once Docker is up | 1 of 7 recovered without spending anything |
