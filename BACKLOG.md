...
|----|-------------|--------------------|-------|
| **WELL-1** | **Declared factor schema — the prerequisite for everything below. ✅ RE-SCOPED 2026-07-26: far cheaper than first assessed.** *Original claim (wrong): the factor name/level is lost because 150 of 155 `design.json` files carry only `{"variant_index": N}`, recoverable only via hand-authored dicts in `generate.py:44`, so it needs a Docker re-index that 117 rows-without-run-dirs would leave ~44 % NULL.* **What is actually true:** `generate.py` writes the human-readable factor **into the condition string**, so the manifest already carries it and **all 60 design keys parse** — 20 as `base\|FACTOR:level` (`ppgpp_conc/basal\|ppGpp:0.6x`, `rrna_operon_knockout/minimal\|rRNA_KO:4op`, `metabolism_kinetic_objective_weight/minimal\|kin_w:1e-6`, `metabolism_secretion_penalty/minimal\|sec_pen:0.05`), 23 as `KO:gene(+gene)`, 15 bare single-factor conditions, 2 timeline specs. **So this is a PARSE + typed-columns job over data we already have — no Docker, no re-index, no NULL column.** Work: (a) a pure `factors(row) -> {family, base_condition, factor, level_raw, level_num, genes[]}` parser with the same "derive from label" discipline as `survey.design_tag`; (b) surface it as manifest columns for new rows AND as a derived view for old ones, so it applies retroactively; (c) stamp explicit factors on newly generated designs so future runs never rely on parsing. ⚠️ Traps: `kin_w:1e-7_default` is not a bare number (level_num must tolerate a suffix); `2op/4op/6op` → 2/4/6; `0.6x` → 0.6; multi-KO `KO:a+b+c` is a SET, not a scalar level — dose-response and set-membership are different query shapes and need different columns. | **Now** — still first, because everything else keys off it | **S/M, not L.** The re-scope removes the expensive part |
