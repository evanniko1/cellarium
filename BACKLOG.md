...
| **DD-ENG-1** | ENG · AGT · TCV | Tool-call error isolation *(verified defect)* | `tools.dispatch` catches only `TypeError`; the `agent.py:441` call site is unguarded → any other tool-body exception (KeyError/ValueError/DuckDB/IO/cobrapy/pydeseq2) propagates, unwinds the whole `converse()` turn, and discards the in-progress answer with **no model-visible error** (can't retry/fallback/report). Fix: broaden the except to return a structured `{"error": …}` the model can read; log the full traceback server-side; tag internal-defect vs transient/data distinctly (don't hide real bugs). Highest-value reliability buy; read-only surface, touches nothing sacred. |
| **DD-MTH-1** | MTH · DS · SCI | Deterministic power grounding in M-8 + df-aware significance *(verified)* | (a) M-8's `underpowered` is an LLM juror's *feel* (`robustness.py:52`) while a real MDE oracle — `tools.power_check`, grounded in corpus replicate CV — sits unused; wire it in as a **fixed, always-run, escalation-blind** step (never verdict-triggered → that's the optional-stopping trap the audit rejected). (b) `rigor.py:93` flags significance at a **df-blind `\|welch_t\|>=2.0`**, contradicting the t-distribution CIs two lines away — switch to a t-critical-at-df threshold. |
| **DD-MTH-2** | MTH · ENG | Reproducibility of the Council control *(verified)* | `council._emit` defaults `temperature=None` (API default) for proposer/skeptic/judge (only the sufficiency gate pins 0). The live server path passes a pinned temperature, but the default is an unpinned latent hole — run-to-run variability of a stated methodological control is a paper reproducibility gap. Pin all Council-role temperatures by default. |
| **DD-ENG-2** | ENG · MTH | Provenance-safe compaction + truncation | Compaction (`agent.py:290-317`) replaces raw history with a Haiku summary with no guarantee grounded numbers survive; oversized tool results are **blind char-sliced at 6000** (`agent.py:447`), which can cut JSON mid-structure and drop the tail of a survey/panel list — both sever the number→tool provenance chain the project exists to preserve. Keep an out-of-band `(number, tool, run_id)` ledger compaction never touches; make truncation structure-aware (drop rows, keep schema + provenance). |
| **DD-ENG-3** | ENG | Loop operational hygiene | `max_turns` inconsistent (converse=12 vs run=8); doc drift ("~25 tools" → 53 real); no loop/no-progress **circuit breaker** (same-tool-same-args repeat / consecutive errors) so a stuck agent doesn't burn the full budget before forced synthesis. |
