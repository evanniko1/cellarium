...
| **DD-SCI-4a** | SCI · AGT · MTH · *2 left* | Multi-KO / reduced-genome generator (`design_generator.py`) | ✅ (a) **`fba_synthetic_lethal` folded in** — the FBA pairwise-lethality test (second oracle over iML1515) runs on the candidates the single-gene prior calls VIABLE (where it's blind); a hit demotes propose→flag + surfaces the reroute-blocker. ✅ (b) **active-learning objective** — `information_gain` scores EXPECTED SURPRISE from the two oracles: `oracle_disagreement` (prior says viable × FBA says synthetic-lethal — dominates), `prior_uncertainty` (surrogate near 0.5), `fba_fragility` (double grows but weakly). `generate(objective="information")` ranks the runs that TEACH the most first (a predicted-viable-yet-FBA-lethal reroute-blocker leads, even though `viability` buries it as 'flag'); glass-box `surprise_why`. Verified live + mock-tested. ⏳ (c) **k>2 completeness** — two distinct gaps at k≥3: (i) *epistasis-check completeness* — the folded-in FBA check is **pairwise only** (`fba_synthetic_lethal` tests `C(k,2)` pairs, never the full set), so it's a sound *lower bound* (a lethal pair kills the superset) but misses the **full k-way FBA deletion** (1 LP: KO all k, check growth — the direct "does this whole build survive?" test) and **higher-order synthetic lethality** (a triple lethal though no pair is); also `fba_fragility` at k≥3 reflects the weakest pair, not the k-way growth. (ii) *search* — replace full enumeration with a principled search (beam / greedy submodular). (d) close the corpus→surrogate→generator→sim→corpus retrain loop *(Phase-2-gated)*. |
