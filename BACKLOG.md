...
| **WELL-6c** | **Graph embeddings (node2vec/DeepWalk) are structurally uninformative here — do not attempt.** Not merely premature: the corpus is a *star*, so every design is one hop from the same control and a random-walk embedding produces near-identical vectors for every leaf. A graph embedding cannot carry information the graph does not have. Revisit only if WELL-1 makes the corpus genuinely factorial, and even then prefer the response-profile vector (WELL-6b), which encodes biology rather than topology. | corpus becomes factorial AND WELL-6b proves insufficient | Default: **never** |
