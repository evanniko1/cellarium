...
| **DD-ENG-2** | ENG · MTH · *1 left* | Provenance-safe compaction + truncation | ✅ (2a, truncation) `_truncate_tool_result` replaces the blind 6000-char slice — it now shrinks the biggest LIST fields (drops rows, keeps scalars + provenance, records the drop-count), so an oversized survey/panel/top_movers stays **valid JSON** instead of being cut mid-structure. ⏳ (2b, compaction ledger) the Haiku history-summary (`agent.py` compaction) still replaces raw history with no guarantee grounded numbers survive — owe an out-of-band `(number, tool, run_id)` ledger compaction never touches. |
