...
| **DD-SCI-4a** | SCI · AGT · MTH · *2 left* | Multi-KO / reduced-genome generator (`design_generator.py`) | ✅ (a) **`fba_synthetic_lethal` folded in** — the FBA pairwise-lethality test (second oracle over iML1515) runs on the candidates the single-gene prior calls VIABLE (where it's blind); a hit demotes propose→flag + surfaces the reroute-blocker. ✅ (b) **active-learning objective** — `information_gain` scores EXPECTED SURPRISE from the two oracles: `oracle_disagreement` (prior says viable × FBA says synthetic-lethal — dominates), `prior_uncertainty` (surrogate near 0.5), `fba_fragility` (double grows but weakly). `generate(objective="information")` ranks the runs that TEACH the most first (a predicted-viable-yet-FBA-lethal reroute-blocker leads, even though `viability` buries it as 'flag'); glass-box `surprise_why`. Verified live + mock-tested. ✅ (c-i) **full k-way deletion** — `fba.fba_gene_deletion` (tool `fba_gene_deletion`) knocks out the WHOLE set at once (1 LP) and reports the set's growth + a `lethal` flag; `design_generator.deletion_check` folds it in at k≥3 (skipped at k=2, where the pair IS the full deletion), so `_fold_epistasis` now demotes a set lethal via a synthetic-lethal PAIR **or** the full k-way deletion (higher-order lethality the pairwise scan misses), and `information_gain` reads the k-way growth/lethality. Verified live (real triple `pfkA+tpiA+gapA` → 0.70, reroutes) + mock-tested. ⏳ (c-ii) *search* — replace full enumeration with a principled search (beam / greedy submodular). ⏳ (d) close the corpus→surrogate→generator→sim→corpus retrain loop *(Phase-2-gated)*. |
