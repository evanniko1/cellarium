...
def shift_response(design: str, channel: str = "ppgpp_conc") -> dict:
    """The ADAPTATION to a declared nutrient shift — the transient a pre/post segment mean throws away."""
...
def similar_designs(design: str, k: int = 8) -> dict:
    ...
    return similarity.similar_designs(design, k)
...
def generate_designs(k: int = 2, max_candidates: int = 8, objective: str = "viability",
                     search: str = "enumerate", beam_width: int | None = None) -> dict:
    """SCI-4: generate + rank reduced-genome / multi-KO candidate designs. objective='viability' (safest first) or
    'information' (active learning — the runs that teach the most first). search='enumerate' | 'greedy' | 'beam'."""
    from . import design_generator
    return design_generator.generate(k=k, max_candidates=max_candidates, objective=objective,
...
TOOLS = [
    ...
    {"name": "trajectory", "description": "The per-GENERATION arc of ONE design: the mean of a channel (growth_rate or ppgpp_conc — the two stored per generation) at each generation index, plus the QC verdict spread there, so a COLLAPSE is visible as the generation where QC stops being all-ok. This is how you INSPECT a lethality case gen-by-gen (e.g. dapA: ppGpp 488→636 over gens 0-1, then division arrest at gen 2). The valid early generations of a collapsing run are shown; the collapsed generation reads as its QC verdict, never a garbage number.",
     "input_schema": {"type": "object", "properties": {"design": {"type": "string", "description": "design label 'perturbation/condition'"}, "channel": {"type": "string", "enum": ["growth_rate", "ppgpp_conc"], "description": "growth_rate or ppgpp_conc (default growth_rate) -- the ONLY two channels stored per generation; anything else needs raw simOut"}}, "required": ["design"]}},
    {"name": "compare_at_generation", "description": "Compare two designs at the SAME generation index (growth_rate or ppgpp_conc) — the 1-to-1 generation comparison of the reporting contract. Unlike the depth-stratified survey, this INCLUDES a collapsing run's valid early generations (a KO that collapses at gen 2 still has trustworthy gen 0/1), and no depth mismatch is possible because both sides are read at one generation. Use to compare a lethal KO to WT at the generation before it collapses.",
     "input_schema": {"type": "object", "properties": {"design_a": {"type": "string"}, "design_b": {"type": "string", "description": "the reference, e.g. wildtype/basal"}, "channel": {"type": "string", "enum": ["growth_rate", "ppgpp_conc"], "description": "growth_rate or ppgpp_conc (default ppgpp_conc) -- the ONLY two channels stored per generation; anything else needs raw simOut"}, "generation": {"type": "integer", "description": "generation index (0 = first, default 0)"}}, "required": ["design_a", "design_b"]}},
    ...
    {"name": "generate_designs", "description": "SCI-4: GENERATE + rank candidate reduced-genome / multi-gene-KO designs, scored by predicted viability — instead of hand-listing sets. Draws a dispensable-gene pool from the corpus (genes observed to tolerate a single KO, minus machinery + benchmark-essential genes), enumerates size-k combinations, and ranks each by a composite: the SCI-5 surrogate's weakest-link viability prior, essentiality/machinery guards, and a biosecurity screen (flagged sets are dropped). DD-SCI-4a: the FBA synthetic-lethal check is now FOLDED IN — run on the candidates the prior calls viable (where it's blind), it catches the pairwise EPISTASIS the single-gene prior can't see (both genes tolerate a single KO but the double abolishes growth); a hit demotes the design to 'flag' with an `epistasis` note and surfaces it as the reroute-blocker to CONFIRM in the sim. Returns runnable multi_gene_knockout Designs (queue via propose_experiments); `fba_available` reports whether the epistasis check ran (needs the `fba` extra). DD-SCI-4a(b): `objective` picks the ranking goal — 'viability' (default: safest reduced-genome builds first) or 'information' (ACTIVE LEARNING: rank by EXPECTED SURPRISE so the runs that teach the most surface first — a predicted-viable-yet-FBA-synthetic-lethal reroute-blocker, or a set the surrogate is unsure about; each carries an `information_gain` + `surprise_why`). Use 'information' when choosing which few runs to spend sim budget on. DD-SCI-4a(c-ii): `search` picks how the space is explored — 'enumerate' (default: score every size-k subset — exact but blows up as C(pool,k)); 'greedy' (build one set gene-by-gene — cheap); 'beam' (keep the top-`beam_width` partial sets, recovering from a bad greedy step). Use greedy/beam for LARGE k or a large pool, where enumeration is infeasible; `n_candidates_scored` reports how many sets were evaluated. `k` = genes per set (default 2).",
     "input_schema": {"type": "object", "properties": {"k": {"type": "integer", "minimum": 2, "description": "genes knocked out per candidate set (default 2; a multi-KO needs >= 2)"}, "max_candidates": {"type": "integer", "description": "how many top-ranked designs to return (default 8)"}, "objective": {"type": "string", "enum": ["viability", "information"], "description": "ranking goal: 'viability' (safest first) or 'information' (active learning — most informative first)"}, "search": {"type": "string", "enum": ["enumerate", "greedy", "beam"], "description": "search strategy: 'enumerate' (exact, C(n,k)); 'greedy' (one incremental build); 'beam' (top-beam_width partial sets — for large k / pool)"}, "beam_width": {"type": "integer", "description": "beam width for search='beam' (default = max_candidates)"}}}},
...
_DISPATCH = {"survey_corpus": survey_corpus, "lethality_landscape": lethality_landscape,
             ...
             "viability_surrogate": viability_surrogate, "generate_designs": generate_designs,
