...
def disconfirm(target: str, reference: str, channel: str) -> dict:
    ...
    tspread = (max(tv) - min(tv)) if len(tv) > 1 else None
    rspread = (max(rv) - min(rv)) if len(rv) > 1 else None
    noise = max(tspread or 0.0, rspread or 0.0)
    within = (abs(tm - rm) <= noise) if (tspread is not None or rspread is not None) else None
    allv = [val(r) for r in rows if val(r) is not None]
    mu, sd = statistics.fmean(allv), (statistics.pstdev(allv) or 1e-12)
    return {
        "channel": channel,
        "target": {"design": target, "mean": round(tm, 6), "n_seeds": len(tv), "values": [round(x, 6) for x in tv]},
        "reference": {"design": reference, "mean": round(rm, 6), "n_seeds": len(rv), "values": [round(x, 6) for x in rv]},
        "effect_pct": (round(100 * (tm - rm) / rm, 1) if rm else None),
        "effect_z_vs_corpus": round((tm - mu) / sd, 2),
        "within_replicate_noise": within,   # True => the effect may be seed noise, not real
        "checklist": [
            "Is the effect larger than the replicate spread (within_replicate_noise should be False)?",
        ...
        "note": "Disconfirmation aid — actively challenge the claimed effect before concluding.",
    }
